...
export function HeroVideo({ video, className = '' }: HeroVideoProps) {
  ...
  // YouTube embed URL with autoplay, muted (required for autoplay), and loop
  const embedUrl = `https://www.youtube.com/embed/${video.id}?autoplay=1&mute=${isMuted ? 1 : 0}&loop=1&playlist=${video.id}&controls=0&modestbranding=1&rel=0&playsinline=1&enablejsapi=1`
    ...
    <>
      ...
      >
        ...
        <div className="group relative aspect-video">
          ...
          {/* Control Overlay - hides on hover to avoid overlap with YouTube's UI */}
          <div className="absolute top-0 left-0 right-0 p-3 bg-gradient-to-b from-black/60 to-transparent transition-opacity duration-300 group-hover:opacity-0 pointer-events-none">
